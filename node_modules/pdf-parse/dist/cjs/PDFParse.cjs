...
const Geometry_js_1 = require('./geometry/Geometry.cjs');
const ImageResult_js_1 = require('./ImageResult.cjs');
const InfoResult_js_1 = require('./InfoResult.cjs');
const PathGeometry_js_1 = require('./PathGeometry.cjs');
const ScreenshotResult_js_1 = require('./ScreenshotResult.cjs');
const TableResult_js_1 = require('./TableResult.cjs');
const TextResult_js_1 = require('./TextResult.cjs');
class PDFParse {
    options;
    doc;
    constructor(options) {
    ...
    static setWorker(workerSrc = undefined) {
        // biome-ignore lint/suspicious/noExplicitAny: <unsupported underline type>
    ...
    async getInfo(params = {}) {
        ...
        const result = new InfoResult_js_1.InfoResult(doc.numPages);
    ...
    async getText(params = {}) {
        const doc = await this.load();
        const result = new TextResult_js_1.TextResult(doc.numPages);
        for (let i = 1; i <= result.total; i++) {
            if (this.shouldParse(i, result.total, params)) {
    ...
    async getImage(params = {}) {
        const doc = await this.load();
        const result = new ImageResult_js_1.ImageResult(doc.numPages);
        for (let i = 1; i <= result.total; i++) {
            if (this.shouldParse(i, result.total, params)) {
                const page = await doc.getPage(i);
    ...
    async getScreenshot(params = {}) {
        ...
        const doc = await this.load();
        const result = new ScreenshotResult_js_1.ScreenshotResult(doc.numPages);
        if (this.doc === undefined) {
    ...
    async getTable(params = {}) {
        const doc = await this.load();
        const result = new TableResult_js_1.TableResult(doc.numPages);
        if (this.doc === undefined) {
            throw new Error('PDF document not loaded');
        }
        for (let i = 1; i <= result.total; i++) {
    ...
    getPathGeometry(mm) {
        ...
        if (mm[0] === Infinity) {
            return PathGeometry_js_1.PathGeometry.undefined;
        }
        if (width > 5 && height > 5) {
            return PathGeometry_js_1.PathGeometry.rectangle;
        }
        else if (width > 5 && height === 0) {
            return PathGeometry_js_1.PathGeometry.hline;
        }
        else if (width === 0 && height > 5) {
            return PathGeometry_js_1.PathGeometry.vline;
        }
        return PathGeometry_js_1.PathGeometry.undefined;
    }
    async getPageTables(page) {
        const lineStore = new Geometry_js_1.LineStore();
        const viewport = page.getViewport({ scale: 1 });
        ...
        for (let i = 0; i < opList.fnArray.length; i++) {
            ...
            if (fn === pdfjs.OPS.constructPath) {
                ...
                if (pg === PathGeometry_js_1.PathGeometry.rectangle) {
                    const rect = new Geometry_js_1.Rectangle(new Geometry_js_1.Point(mm[0], mm[1]), mm[2] - mm[0], mm[3] - mm[1]);
                    rect.transform(transformMatrix);
                    rect.transform(viewport.transform);
                    lineStore.addRectangle(rect);
                }
                else if (pg === PathGeometry_js_1.PathGeometry.hline || pg === PathGeometry_js_1.PathGeometry.vline) {
                    const from = new Geometry_js_1.Point(mm[0], mm[1]);
                    const to = new Geometry_js_1.Point(mm[2], mm[3]);
                    const line = new Geometry_js_1.Line(from, to);
...
PDFParse.setWorker();
//# sourceMappingURL=PDFParse.cjs.map