...
export class OpenAIAgentService {
  ...
  private static extractToolCalls(response: any): Array<{ id: string; name: string; arguments: any }> {
    if (!response) {
      return [];
    }

    const toolCallMap = new Map<string, { id: string; name: string; arguments: any }>();
  ...
  private static async submitToolOutputs(options: {
    responseId: string;
    requestId: string;
    ...
    try {
      const followUpResponse = await axios.post(
        `${this.OPENAI_BASE_URL}/responses/${options.responseId}/submit_tool_outputs`,
